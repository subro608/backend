...
from accounts.models import Lessor, User
import os
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
...
class PropertyWishlistView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if not serializer.is_valid():
                return Response(
                    {"success": False, "error": True, "message": serializer.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {
                    ...
                    "data": {
                        ...
                        "message": (
                            ...
                            if wishlist_item.is_wishlist
                            else "Property removed from wishlist"
                        ),
                    },
                },
                status=status.HTTP_200_OK,
...
class GetAllPropertiesView(APIView):
    ...
    def get(self, request):
        ...
        properties_query = Properties.objects.filter(is_deleted=False).order_by(
            "-created_at"
        )
        paginator = Paginator(properties_query, per_page)
...
class GetMyListings(APIView):
    ...
    def get(self, request):
        ...
        if not Lessor.objects.filter(user=user).exists():
            return Response(
                {"success": False, "error": True, "message": "Not a valid lessorr"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        # Query properties data ordered by `created_at`
        properties_query = Properties.objects.filter(lessor_id=user.id).order_by(
            "-created_at"
        )
        paginator = Paginator(properties_query, per_page)
...
class SubmitPropertyForVerificationView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if (
                property_obj.status_verification
                == self.STATUS_VERIFICATION_PROPERTY_SUBMITTED
            ):
                return Response(
            ...
            print(
                f"Error in SubmitPropertyForVerificationView: {str(e)}"
            )  # Add logging
            return Response(
        ...
        try:
            ...
            try:
                wishlist_item = PropertyWishlist.objects.get(
                    lessee_id=lessee_id, property_id=property_id, is_wishlist=True
                )
    ...
    def post(self, request):
        try:
            ...
            amenities_obj, created = PropertyAmenities.objects.get_or_create(
                ...
                defaults={"created_at": timezone.now(), "modified_at": timezone.now()},
            )
...
class GetPropertyDetailsView(APIView):
    ...
    def get(self, request, property_id):
        ...
        # Original query
        property_obj = Properties.objects.filter(id=property_id, is_deleted=False)

        if user_role == 3:
            lessor = Lessor.objects.get(user=user)
            property_obj = property_obj.filter(lessor_id=lessor.user_id)

        property_obj = property_obj.first()
    ...
    def get(self, request):
        ...
        try:
            ...
            if lessee_id:
                wishlist_items = {
                    str(id)
                    for id in PropertyWishlist.objects.filter(
            ...
            if not geocode_result:
                return Response(
                    {"success": False, "error": True, "message": "Location not found"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            if sort_by and sort_by in sort_options:
                ...
                has_filters = any(
                    [min_rent, max_rent, bedrooms, bathrooms, property_type]
                )
                if has_filters:
            ...
            for prop in properties:
                ...
                a = (
                    ...
                    * math.cos(math.radians(float(prop.latitude)))
                    * math.sin(dlon / 2) ** 2
                )
